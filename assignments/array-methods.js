...
let fullName = [[runners.first_name] + [runners.last_name]];
 let result = fullName.map(function(runners) {
...
//  Q&A way
  let allCaps = runnners.map(item=>item.first_name.toUpperCase());
  console.log(allCaps);
...
let largeShirts = [];
runners.filter( large => large.shirt_size === "L");
...
let allLower = [];
runners.map(lower => allLower.push(lower.first_name.toLowerCase()));
